...
import { View, Text, TextInput, TouchableOpacity, ScrollView, Alert, ActivityIndicator, Dimensions, ToastAndroid } from "react-native";
...
export default function Address() {
  ...
  const detectLocation = async () => {
    try {
      ...
      if (geocode && geocode.length > 0) {
        ...
        const city = (place.city || "").toString().trim().toLowerCase();
        ...
        if (city === "vijayawada") {
          ...
          showToast("Service available in Vijayawada");
        } else {
          Alert.alert(
            "Sorry",
            `We are not providing Scrap Express service in ${place.city || "this"} city`
          );
        }
      }
          } catch (err) {
  ...
  const createAddress = async () => {
    ...
    const city = (form.city || "").toString().trim().toLowerCase();
    if (city!=="vijayawada") {
      Alert.alert(
        "Sorry",
        `We Don't Provide Scrap Express services for ${form.city || "this"} city`
      );
      return;
    }
